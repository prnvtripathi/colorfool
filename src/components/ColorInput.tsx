...
export default function ColorInput() {
    ...
    return (
        <div className="flex flex-col gap-2 w-full">
            <label htmlFor="color-input" className="font-medium text-sm sr-only">
                Enter a color code
            </label>
            <Input
                id="color-input"
                ...
                className="text-lg px-4 py-3 rounded-lg border border-input bg-background focus:outline-none focus:ring-2 focus:ring-primary transition-colors"
            />
            {error && (
                <span id="color-error" role="alert" aria-live="polite" className="text-destructive text-xs mt-1">
                    {error}
                </span>
            )}
        </div>
